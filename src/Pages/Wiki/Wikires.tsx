...
const Wikires = () => {
  ...
  if (!data || !data.parse) {
    return (
      <div className="flex flex-col items-center justify-center p-10 gap-4 my-10 text-lime-400">
        <h2 className="text-lg font-bold">
          Desculpe, não foi possível estabelecer uma conexão. Tente novamente mais tarde.
        </h2>
        <i className="fa-regular fa-face-sad-tear text-xl"></i>
        <button 
          onClick={() => window.history.back()} 
          className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-lime-700 rounded-lg hover:bg-lime-800 focus:ring-4 focus:outline-none focus:ring-lime-700 gap-2"
        >
          Voltar para página anterior <i className="fa-solid fa-rotate-left"></i>
        </button>
      </div>
    );
  }
