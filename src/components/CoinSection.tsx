    ...
    <section className='flex flex-col gap-4 px-[8em]'>
        <div className='flex gap-3 items-center'>
            ...
            <span className='w-2 h-2 bg-lime-400 animate-pulse rounded-full'></span>
                ...
                <tr>
                <th scope='col' className='px-6 py-3'><p>Moeda</p></th>
                <th scope='col' className='px-6 py-3'><p>Preço</p></th>
                        <th scope='col' className='px-6 py-3'><p>Alta</p></th>
                        <th scope='col' className='px-6 py-3'><p>Baixa</p></th>
                        <th scope='col' className='px-6 py-3'><p>Mudança</p></th>
                    </tr>
<p></p>
                </thead>
            ...
            {data && Object.values(data).map((item) => (<>
                <tr className='border-b bg-slate-200 odd:bg-slate-300 text-[#1E1E1E]'>
                    ...
                    <td className='px-6 py-4'><p>{item.code!} {item.name.replace('/Real Brasileiro','')}</p></td>
                    <td className='px-6 py-4'><p>{currency(item.bid,item.code)}</p></td>
                    <td className='px-6 py-4'><p>{currency(item.high,item.code)}</p></td>
                    <td className='px-6 py-4'><p>{currency(item.low,item.code)}</p></td>
                    <td className='px-6 py-4' style={ 
                        item.pctChange < 0 ? { color: 'red' } : 
                        item.pctChange > 0 ? 
                        { color: 'green' } : 
                        { color: '#1E1E1E' } }><p>{item.pctChange}%</p></td>
                </tr>
            </>))}
                 </tbody>
